...
{
if (key <= 0) {
throw invalid_argument("Ключ должен быть положительным
числом");
}
columns = key;
}
...
{
if (plain_text.empty()) {
throw std::invalid_argument("Текст не может быть
пустым");
}
int text_length = plain_text.length();
int rows = (text_length + columns - 1) / columns;
vector<vector<wchar_t>> table(rows,
vector<wchar_t>(columns, L' '));
int index = 0;
...
throw invalid_argument("Некорректная длина
